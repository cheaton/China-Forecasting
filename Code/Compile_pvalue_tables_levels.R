...
# test for the forecasts computed from models estimated with the data in levels, 
# and creates a list named p.value.levels.tables.'variable' for each variable
# e.g. p.value.levels.tables.CPI. It also saves tables of p-values in 
# ../Output/pvalue_tables.

if (whereami){cat("\nConstructing tables of p-values for levels")}
...
for (variable in variable.names)
{
  ...
  cmd <- paste("p.value.levels.tables.",variable," <- list(conditional=conditional, unconditional=unconditional)",sep="")
  eval(parse(text=cmd))
  ...
  for (crit in crits)
  {
    cmd <- paste("write.csv(",crit,".conditional.pvalue.table, '../Output/pvalue_tables/",variable,"_",crit,"_levels_conditional.pvalues.csv')",sep="")
    eval(parse(text=cmd))
    cmd <- paste("write.csv(",crit,".unconditional.pvalue.table, '../Output/pvalue_tables/",variable,"_",crit,"_levels_unconditional.pvalues.csv')",sep="")
    eval(parse(text=cmd))
    
 
    table.heading <- paste(variable,": Conditional p-values by Horizon")
    cmd <- paste("print.xtable(xtable(",crit,".conditional.pvalue.table[,horizons],digits=3,caption='",table.heading,"'),file='../Output/pvalue_tables/",variable,"_",crit,"_levels_conditional.pvalues.tex',caption.placement='top',booktabs=TRUE)",sep="")
    eval(parse(text=cmd))
    table.heading <- paste(variable,": Unconditional p-values by Horizon")
    cmd <- paste("print.xtable(xtable(",crit,".unconditional.pvalue.table[,horizons],digits=3,caption='",table.heading,"'),file='../Output/pvalue_tables/",variable,"_",crit,"_levels_unconditional.pvalues.tex',caption.placement='top',booktabs=TRUE)",sep="")
    eval(parse(text=cmd))
  }
}
